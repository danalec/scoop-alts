    ...
    def generate_manifest(self, config: SoftwareConfig) -> Dict[str, Any]:
        ...
        if config.pre_install:
            manifest["pre_install"] = [
                cmd.replace("$version", version) for cmd in config.pre_install
            ]

        if config.post_install:
            manifest["post_install"] = [
                cmd.replace("$version", version) for cmd in config.post_install
            ]
        ...
        if getattr(config, "persist", None):
            if isinstance(config.persist, list):
                manifest["persist"] = [
                    p.replace("$version", version) if isinstance(p, str) else p
        ...
        manifest["checkver"] = {"url": config.homepage, "regex": config.version_regex}

        if config.download_url_template:
            manifest["autoupdate"] = {"url": config.download_url_template}

...
def update_manifest():
    ...
    if not version_info:
        if not structured_only:
            print("⚠️  Shared version detection failed, trying GitHub API fallback...")
        try:
            ...
            asset_pattern = "ungoogled-chromium_.*_windows_x64.zip"
            matching_assets = [
                a
            ...
            version_info = {"version": version, "download_url": download_url, "hash": hash_value}
            if not structured_only:
